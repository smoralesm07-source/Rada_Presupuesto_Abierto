...
SEARCH_BLOB_SQL = """
strip_accents(concat_ws(' ',
  coalesce(nombre_beneficiario,''), coalesce(rut_beneficiario,''),
  coalesce(beneficiario_source_id,''), coalesce(nombre_partida,''),
  coalesce(nombre_capitulo,''), coalesce(nombre_area,''),
  coalesce(nombre_subtitulo,''), coalesce(nombre_item,''),
  coalesce(nombre_asignacion,''), coalesce(nombre_programa_presupuestario,''),
  coalesce(nombre_bip,''), coalesce(nombre_ubicacion_geografica,''),
  coalesce(numero_documento,''), coalesce(tipo_documento,''),
  coalesce(orden_compra,''), coalesce(sector,''), coalesce(region,'')
))
""".strip()
...
def build_fts(df: pd.DataFrame, db_path: str | Path) -> None:
    ...
    for _, row in df.iterrows():
        budget = " ".join(
            _txt(row.get(c))
            for c in [
                "nombre_subtitulo", "nombre_item", "nombre_asignacion",
                "nombre_programa_presupuestario", "nombre_bip",
                "nombre_ubicacion_geografica",
            ]
        )
        institution = " ".join(
            _txt(row.get(c)) for c in ["nombre_partida", "nombre_capitulo"]
        )
        rows.append(
            (
    ...
    con.executemany(f"INSERT INTO transactions_fts VALUES ({placeholders})", rows)
    con.commit()
    con.close()
...
def query_fts(db_path: str | Path, query: str, limit: int = 50) -> list[dict]:
    ...
    rows = con.execute(
        "SELECT *, bm25(transactions_fts) AS rank "
        "FROM transactions_fts WHERE transactions_fts MATCH ? "
        "ORDER BY rank LIMIT ?",
        (query, max(1, min(int(limit), 10_000))),
    ).fetchall()
    con.close()
    return [dict(r) for r in rows]


def build_fts_from_parquet(
    parquet_glob: str,
    db_path: str | Path,
    ...
    sqlcon.execute("PRAGMA journal_mode=OFF")
    sqlcon.execute("PRAGMA synchronous=OFF")
    sqlcon.execute("DROP TABLE IF EXISTS transactions_fts")
    sqlcon.execute(_fts_sql())
    dcon = duckdb.connect()
    q = f"""
    SELECT transaction_id, organization_id, recipient_id, coalesce(provider_id,''),
           coalesce(rut_beneficiario,''), coalesce(beneficiario_source_id,''),
           coalesce(nombre_beneficiario,''),
           concat_ws(' ',coalesce(nombre_partida,''),coalesce(nombre_capitulo,'')),
           coalesce(nombre_area,''),
           concat_ws(' ',coalesce(nombre_subtitulo,''),coalesce(nombre_item,''),
                     coalesce(nombre_asignacion,''),coalesce(nombre_programa_presupuestario,''),
                     coalesce(nombre_bip,''),coalesce(nombre_ubicacion_geografica,'')),
           coalesce(numero_documento,''), coalesce(tipo_documento,''),
           coalesce(orden_compra,''), coalesce(codigo_bip,''),
           coalesce(sector,''), coalesce(region,'')
    ...
    while True:
        ...
        if batch.empty:
            break
        sqlcon.executemany(
            f"INSERT INTO transactions_fts VALUES ({placeholders})",
            batch.itertuples(index=False, name=None),
        )
        total += len(batch)
        if total % 250_000 < len(batch):
            sqlcon.commit()
    sqlcon.commit()
    sqlcon.close()
    dcon.close()
...
def hybrid_search(
    ...
    provider_only: bool | None = None,
    person_only: bool | None = None,
    ...
    """Parameterized, accent-insensitive and token-aware search over normalized facts."""
    import duckdb
    ...
    if text:
        ...
        for token in tokens:
            where.append(f"lower({SEARCH_BLOB_SQL}) LIKE ?")
            params.append(f"%{token}%")

    if rut:
        nrut = normalize_rut(rut)
        if not nrut:
            raise ValueError("RUT inválido: formato o dígito verificador")
        where.append("rut_beneficiario = ?")
        params.append(nrut)
    ...
    if date_from:
        where.append(
            "coalesce(try_cast(fecha_documento AS DATE),try_cast(fecha_pago AS DATE)) >= ?::DATE"
        )
    ...
    for field, value, operator in [
        ("monto_devengado", min_amount, ">="),
        ("monto_devengado", max_amount, "<="),
        ("monto_pago", min_paid_amount, ">="),
        ("monto_pago", max_paid_amount, "<="),
    ]:
        if value is not None:
            where.append(f"try_cast({field} AS DOUBLE) {operator} ?")
            params.append(float(value))

    for field, value in [
        ("nombre_ubicacion_geografica", location),
        ("region", region),
        ("sector", sector),
        ...
        if value:
            ...
            where.append(f"lower(strip_accents(coalesce({field},''))) LIKE ?")
            params.append(f"%{normalized_value}%")

    if budget_code:
        where.append(
            "concat_ws('.',coalesce(subtitulo,''),coalesce(item,''),coalesce(asignacion,'')) LIKE ?"
        )
        params.append(f"{budget_code}%")

    for field, value in [
        ("is_provider", provider_only),
        ("is_person", person_only),
    ...
    df = con.execute(
        ...
        FROM read_parquet('{parquet_glob}', union_by_name=true)
        WHERE {clause}
        ORDER BY periodo DESC, mes DESC,
                 try_cast(monto_devengado AS DOUBLE) DESC NULLS LAST
        LIMIT {safe_limit}
        """,
        params,
    ).df()
...
def main() -> None:
    ...
    p.add_argument("--db", default="data/index/search.sqlite")
