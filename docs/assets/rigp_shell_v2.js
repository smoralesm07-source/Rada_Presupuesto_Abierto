...
function renderHome(){
  ...
  const allCases=readCases(),cases=activeCases(),rows=prioritizedRows(),caseFindings=new Set(allCases.flatMap(c=>c.finding_ids||[]));
...
function aggregate(mode){
  ...
  for(const r of shell.rows){
    const isProvider=mode==='provider',id=isProvider?r.provider_id:r.organization_id,name=isProvider?r.provider_name:r.organization_name;
    if(!id&&!name)continue;
    const key=String(id||name);let x=map.get(key);
    ...
    if(counterpart)x.counterparties.add(String(counterpart));if(r.periodo)x.periods.add(String(r.periodo));x.findings+=1;
    if(r.attention_level==='ATENCION_INMEDIATA')x.immediate+=1;x.maxScore=Math.max(x.maxScore,Number(r.max_priority_score||0));x.maxAmount=Math.max(x.maxAmount,Number(r.max_transaction_amount||0));
  }
...
function renderExplore(){
  const workspace=$('workspace');if(!workspace)return;if(!shell.loaded){workspace.innerHTML='<div class="empty large"><h2>Cargando explorador</h2></div>';return}
  const label=shell.exploreMode==='provider'?'proveedores':'servicios',q=shell.exploreQuery.trim().toLocaleLowerCase('es'),all=aggregate(shell.exploreMode);
...
function routeToTriage(query=''){
  const btn=document.querySelector('#primaryNav [data-view="triage"]');if(!btn)return;btn.click();
...
function maybeStart(){
  if(shell.started||!shell.loaded)return;const workspace=$('workspace'),status=$('status');
  if(!workspace||!workspace.childNodes.length||!status||!/relaciones|arquitectura/i.test(status.textContent))return;
  shell.started=true;document.querySelector('#primaryNav [data-view="inicio"]')?.click();
}
...
if(workspace)observer.observe(workspace,{childList:true});if(banner)observer.observe(banner,{attributes:true,childList:true,subtree:true});if(status)observer.observe(status,{childList:true,characterData:true,subtree:true});
caseRepo?.subscribe?.(()=>{if(shell.current==='inicio')renderHome();syncCaseNav()});
...
fetch(DATA_URL,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('HTTP '+r.status);return r.json()}).then(d=>{shell.payload=d;shell.rows=Array.isArray(d.relation_findings)?d.relation_findings:[];shell.loaded=true;maybeStart()}).catch(()=>{shell.payload=null;shell.rows=[];shell.loaded=true;maybeStart()});
