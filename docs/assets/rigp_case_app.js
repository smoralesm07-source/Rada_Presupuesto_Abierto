...
const SIGNAL_LABEL={
  PROVIDER_CONCENTATION:'Concentración en proveedor',
...
const FAMILY_LABEL={
  ...
  INTEGRIDAD_DOCUMENTAL_PAGOS:'Integridad documental y pagos',
...
const EVIDENCE_TEMPLATE=[
  ['PRESUPUESTO','Hecho presupuestario','Confirmar monto, período, programa y trazabilidad del registro fuente.'],
  ['CONTRATACION','Contratación / orden de compra','Identificar proceso, orden de compra, modalidad, competencia y adjudicación.'],
  ['PAGO','Ejecución y pagos','Revisar recepción conforme, facturas, fechas, modificaciones y secuencia de pagos.'],
  ['ENTIDAD','Identidad y capacidad del proveedor','Confirmar RUT/identidad, giro, antigüedad, tamaño y capacidad observable.'],
  ['PROPIEDAD','Propiedad y actores','Resolver socios, representantes y vínculos relevantes sólo con fuentes trazables.'],
  ['CONTEXTO','Contexto externo','Contrastar CGR, SII, sanciones u OSINT sin tratar coincidencias candidatas como hechos.']
];

const S={
  rows:[],payload:null,view:'bandeja',query:'',level:'ACTIVE',page:0,caseId:null,
  entityMode:'provider',contextLoaded:false,contextLoading:false,
  entityContext:null,procurementContext:null
};
...
function defaultHypothesis(r){
  switch(r.finding_family){
    ...
    case'INTEGRIDAD_DOCUMENTAL_PAGOS':return 'Determinar si la reiteración documental corresponde a hitos válidos o si existe duplicidad material que requiera revisión adicional.';
    case'CONVERGENCIA_MULTIFACTOR':return 'Reconstruir la relación servicio–proveedor y establecer si la convergencia de patrones tiene una explicación operativa suficiente o requiere profundización.';
    case'CONCENTRACION_DEPENDENCIA':return 'Evaluar si la concentración responde a estructura de mercado, contrato o convenios legítimos, y si existen factores adicionales que justifiquen atención reforzada.';
    case'IRRUPCION_CAMBIO_ESCALA':return 'Verificar si el cambio de escala del proveedor tiene sustento contractual y capacidad observable compatibles con el gasto registrado.';
    default:return 'Revisar el hallazgo en contexto, contrastando documentos y fuentes antes de formular cualquier conclusión.';
  }
}
...
function filteredRows(){
  ...
  return S.rows.filter(r=>{
    ...
    if(!q)return true;
    return [r.organization_name,r.organization_id,r.provider_name,r.provider_id,r.finding_title,r.finding_family,r.periodo,...(r.signal_types||[])].join(' ').toLocaleLowerCase('es').includes(q);
  }).sort((a,b)=>rank(a)-rank(b)||Number(b.max_priority_score||0)-Number(a.max_priority_score||0)||Number(b.signal_family_count||0)-Number(a.signal_family_count||0));
}
...
document.addEventListener('click',e=>{
  const v=e.target.closest('[data-view]');if(v){S.view=v.dataset.view;S.page=0;render();return}
  const create=e.target.closest('[data-create-case]');if(create){const r=findingById(create.dataset.createCase);if(r){ensureCase(r);S.view='caso';render()}return}
  const open=e.target.closest('[data-open-case]');if(open){S.caseId=open.dataset.openCase;S.view='caso';render();return}
  const st=e.target.closest('[data-case-status]');if(st&&S.caseId){patchCase(S.caseId,{status:st.dataset.caseStatus},'ESTADO_CAMBIADO');render();return}
  const pg=e.target.closest('[data-page]');if(pg){S.page+=Number(pg.dataset.page||0);render();return}
  const em=e.target.closest('[data-entity-mode]');if(em){S.entityMode=em.dataset.entityMode;render();return}
