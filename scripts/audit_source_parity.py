...
UA={"User-Agent":"RadarPresupuestoAbierto/3.0 (+public source parity audit)"}
...
def official_providers(year: int):
    # El ranking usa el año seleccionado en la propia página; el parámetro se conserva
    # como metadato porque el HTML inicial suele cargar el año vigente.
...
def local_metrics(parquet:str,year:int):
    ...
    # Aproximación explícita a la vista Proveedor/Receptor: flag de fuente, RUT válido y exclusión ST21.
    pr=con.execute("""SELECT count(DISTINCT rut_beneficiario),sum(coalesce(monto_devengado,0)) FROM f WHERE periodo=? AND is_provider=TRUE AND coalesce(rut_beneficiario,'')<>'' AND ltrim(coalesce(subtitulo,''),'0')<>'21'""",[year]).fetchone()
    top=con.execute("""SELECT rut_beneficiario,arg_max(nombre_beneficiario,abs(monto_devengado)) name,sum(coalesce(monto_devengado,0)) amount_clp FROM f WHERE periodo=? AND is_provider=TRUE AND coalesce(rut_beneficiario,'')<>'' AND ltrim(coalesce(subtitulo,''),'0')<>'21' GROUP BY 1 ORDER BY 3 DESC LIMIT 20""",[year]).fetchall()
    con.close()
    return {"year":year,"latest_devengo_month":str(latest) if latest else None,"latest_payment_date":str(latestpay) if latestpay else None,"transactional_services":int(base[0] or 0),"areas":int(base[1] or 0),"devengo_clp":float(base[2] or 0),"payment_clp":float(base[3] or 0),"provider_receiver_proxy":{"definition":"PROVEEDOR=1 + RUT válido + subtítulo != 21","distinct_ruts":int(pr[0] or 0),"devengo_clp":float(pr[1] or 0),"top":[{"rut":r[0],"name":r[1],"amount_clp":float(r[2] or 0)} for r in top]}}
...
def main():
    ...
    payload={"generated_at":datetime.now(timezone.utc).isoformat(timespec='seconds'),"source":"Presupuesto Abierto - DIPRES","year":a.year,"official":{"status":status,"providers":providers,"bulk":head},"local":local,"top_provider_matches":matches,"assessment":{"service_grain":"PARTIDA_CAPITULO","area_separated":True,"payments_use_fecha_pago_monto_pago":True,"provider_comparison_is_proxy":True,"note":"La comparación proveedor/receptor es una aproximación trazable al criterio público. Diferencias pueden persistir por reglas internas de la web, fecha de actualización o ajustes de fuente."}}
    ...
    print('[OK] parity',json.dumps({"official_services":status.get('services_total'),"official_transactional":status.get('services_transactional'),"local_services":local.get('transactional_services'),"official_provider_total":providers.get('total_displayed'),"local_provider_proxy":local.get('provider_receiver_proxy',{}).get('distinct_ruts'),"latest_devengo":local.get('latest_devengo_month'),"latest_payment":local.get('latest_payment_date'),"top_matches":len(matches)},ensure_ascii=False))
